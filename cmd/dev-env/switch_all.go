...
Examples:
  ...
  gz dev-env switch-all --env production --dry-run

  # Switch using environment file
  gz dev-env switch-all --from-file production.yaml

  # Interactive environment selection
  gz dev-env switch-all --interactive

  # Force switch without confirmation
  gz dev-env switch-all --env dev --force`,
...
func (opts *switchAllOptions) run(ctx context.Context) error {
	...
	if err := opts.registerServiceSwitchers(switcher); err != nil {
		return fmt.Errorf("failed to register service switchers: %w", err)
	}
...
func (opts *switchAllOptions) registerServiceSwitchers(switcher *devenv.EnvironmentSwitcher) error {
	...
	gcpSwitcher := &GCPSwitcher{}
	switcher.RegisterServiceSwitcher("gcp", gcpSwitcher)

	// Register Azure switcher
	azureSwitcher := &AzureSwitcher{}
	switcher.RegisterServiceSwitcher("azure", azureSwitcher)

	// Register Docker switcher
	dockerSwitcher := &DockerSwitcher{}
	switcher.RegisterServiceSwitcher("docker", dockerSwitcher)

	// Register Kubernetes switcher
	kubernetesSwitcher := &KubernetesSwitcher{}
	switcher.RegisterServiceSwitcher("kubernetes", kubernetesSwitcher)

	// Register SSH switcher
	sshSwitcher := &SSHSwitcher{}
	switcher.RegisterServiceSwitcher("ssh", sshSwitcher)
