...
func TestAnalyzeProfileGoroutine(t *testing.T) {
	...
	err := os.WriteFile(goroutineProfile, []byte("fake goroutine profile data"), 0644)
	require.NoError(t, err)
...
func TestEnhancedCommandsIntegration(t *testing.T) {
	...
	for _, file := range testFiles {
		err := os.WriteFile(filepath.Join(tempDir, file), []byte("fake profile data"), 0644)
		require.NoError(t, err)
	}
...
func TestPerformanceIssueTypes(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run("issue type "+tc.expectedType, func(t *testing.T) {
			...
			err := os.WriteFile(testFile, []byte("fake profile data"), 0644)
			require.NoError(t, err)
...
func BenchmarkAnalyzeProfile(b *testing.B) {
	...
	err := os.WriteFile(profileFile, []byte("fake profile data"), 0644)
	if err != nil {
		b.Fatal(err)
	}
