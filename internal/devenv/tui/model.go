...
func NewModel(ctx context.Context) *Model {
	...
	checkers := []status.ServiceChecker{
		status.NewAWSChecker(),
		status.NewGCPChecker(),
		status.NewAzureChecker(),
		status.NewDockerChecker(),
		status.NewKubernetesChecker(),
		status.NewSSHChecker(),
	}
...
func (m *Model) updateCurrentView(msg tea.Msg) tea.Cmd {
	...
	case ViewServiceDetail:
		...
		// Track: https://github.com/Gizzahub/gzh-cli/issues/XXX
		return nil
	case ViewEnvironmentSwitch:
