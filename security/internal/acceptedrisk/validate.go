...
// verifiedCommit is what a commit verifier reports about one approval commit.
type verifiedCommit struct {
	SHA      string
	Verified bool
}

// commitVerifier reports whether an approval commit carries a signature that
// verifies. Implementations wrap `git verify-commit`; this package requires one
// rather than shelling out itself so that the authority rules stay independent of
// any particular repository access mechanism.
...
func approvalViolations(current policy, entry record, verifier commitVerifier) []violation {
	...
	violations := make([]violation, 0)
	if approver.Type != approverTypeUser || isNonHumanLogin(approver.Login) || isNonHumanLogin(entry.Approver.Login) {
		violations = append(violations, violation{
			Code:    codeApproverNotHuman,
			Subject: entry.ID,
			Detail:  fmt.Sprintf("approver id %d resolves to an automation or agent identity", entry.Approver.ID),
		})
	}
	...
	return append(violations, evidenceViolations(current, entry, verifier)...)
}

func evidenceViolations(current policy, entry record, verifier commitVerifier) []violation {
